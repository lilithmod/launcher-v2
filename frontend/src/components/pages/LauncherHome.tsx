		...
		<div
			...
			<pre tw='drop-shadow-md text-neutral-200 text-sm font-medium font-mono'>
				{/*{Logs.map((item: string) => {
					return (
						<div
							dangerouslySetInnerHTML={{ __html: parser.toHtml(item.replace('{*lilith_redraw_line*}', '')) }}
						/>
					)
				})}*/}
			</pre>
			...
			<div tw='h-screen bg-cover' style={{ backgroundImage: `url(data:image/jpeg;base64,${BackgroundImage})` }}>
				<div tw='pt-36 pl-20'>
					...
					<p
						...
						// css={
						// 	ButtonStartStatus &&
						// 	tw`-translate-y-16 mt-1 -translate-x-12 text-lg font-bold text-neutral-200 duration-300 transition`
						// }
					>
								...
								<p tw='duration-300 transition font-black text-2xl text-white group-hover:drop-shadow'>
									{/*{!ButtonStartStatus && (
										<svg
											...
											viewBox='0 0 24 24'
											fill='currentColor'
											className='w-6 h-6 inline mr-2 -mt-0.5'>
											<path
												fillRule='evenodd'
				...
				<div>
					{/*css={ButtonStartStatus && tw`hidden`}*/}
					<div tw='absolute bottom-2 left-20 z-0'>
						<button
							onClick={() => BrowserOpenURL('https://discord.com/invite/lilith')}
							tw='transition text-neutral-300 opacity-60 hover:opacity-100'>
