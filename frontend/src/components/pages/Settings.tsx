...
import { BadgeCheckIcon, ExclamationIcon, ExternalLinkIcon, TrashIcon } from '@heroicons/react/outline'
...
import { DeleteEverything, GetVersion, SaveConfig, ShowDialog } from '../../../wailsjs/go/main/App'
import { BrowserOpenURL } from '../../../wailsjs/runtime'
import { Sparkles } from '../../assets/images'
import { PageContentBlock, Shimmer, Spinner } from '../../components/elements/Generic'
import { Action, Modal } from '../../components/elements/Modal'
		...
		<PageContentBlock pageId={props.id}>
			...
			<Modal
				...
				content={{
					...
					description:
						'All files related to Lilith have been removed. You will need to log in to your Microsoft account on next usage.',
					button: 'Got it!',
					function: () => {
						setDone(false)
						navigate('/')
			...
			<div
				onClick={() => {
					...
					ShowDialog(
						`Lilith Launcher v${version}`,
						'© 2021-2024 theMackabu, Lilith Development',
						['Ok'],
						'Ok',
						'',
						'',
			...
			<div tw='max-w-2xl mx-auto px-6 py-8 w-full pt-16'>
				<div tw='relative mt-4'>
					{!loaded && (
						<div tw='absolute z-10 top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 opacity-100 flex flex-col items-center justify-center text-center bg-black/10 rounded-lg px-3 py-5 border border-neutral-600/50 backdrop-blur'>
							<Spinner size='base' />
							<p tw='text-neutral-100 text-sm font-medium mt-3 w-96'>
								Please start Lilith to generate a config file to continue.
							</p>
						</div>
					)}
					<div
								...
								{config.nicknames && Object.entries(config.nicknames).length > 0
									? Object.entries(config.nicknames).map(([key, value]: any, idx: number) => (
											<span
												...
												key={idx}>
												<span>
													<span tw='font-semibold'>{key}</span> is nicked as{' '}
						...
						<div tw='flex items-center justify-between'>
							<span tw='flex-grow flex flex-col'>
								...
								<span className='text-sm text-neutral-400'>
									Lilith beta builds may not be available for all users.
								</span>
							</span>
							<Listbox
									...
									<>
										...
										<div tw='mt-1 relative'>
											<Listbox.Button tw='bg-neutral-900/30 hover:bg-neutral-700/30 transition relative w-40 border border-neutral-700 hover:border-neutral-600 cursor-pointer rounded-md shadow-sm pl-3 pr-10 py-2 text-left focus:outline-none focus:ring-1 focus:ring-rose-500 focus:border-rose-500 sm:text-sm'>
												<span tw='block truncate text-neutral-50'>
													{channels[config.alpha ? 1 : 0].name}
												</span>
															...
															{({ selected, active }) => (
																<>
																	...
																	{selected ? (
																		<span
																			css={[
																				active ? tw`text-white` : tw`text-rose-300`,
				...
				<div tw='border border-neutral-700/50 bg-neutral-800 shadow px-4 py-5 rounded-lg sm:p-6 mt-4 w-full'>
					...
					<div className='relative group'>
						...
						<img
							src={Sparkles}
							className='absolute -top-3 -right-4 opacity-0 group-hover:opacity-100 w-8 transition'
						/>
						<img
							src={Sparkles}
							className='absolute bottom-2 -left-5 opacity-0 group-hover:opacity-100 w-8 transition'
						/>
					</div>
				</div>
				<div tw='border border-neutral-700/50 bg-neutral-800 shadow px-4 py-5 rounded-lg sm:p-6 mt-4 w-full'>
					...
					<button
						...
						<p tw='transition flex items-center px-2 py-2 text-xl font-bold rounded-md text-red-200 group-hover:text-white'>
							<TrashIcon
								className='transition group-hover:text-neutral-100 text-red-200 mr-3 flex-shrink-0 h-5 w-5'
								aria-hidden='true'
							/>
							Delete Everything
						</p>
