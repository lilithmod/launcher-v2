...
const NavLink = (props: { to: string; name: any }) => {
	...
	return (
		<Link
			...
			css={[
				LinkStyle,
				location.pathname.startsWith(`/${props.to.split('/')[1]}`) && tw`after:w-full pointer-events-none`,
			]}>
			{props.name}
		</Link>
	)
	...
	return (
		<div
			...
			<div tw='flex justify-between items-center py-2 space-x-6'>
				...
				<button type='button' onClick={() => BrowserOpenURL('https://lilith.rip/pricing')} css={LinkStyle}>
					Plans
				</button>
